...
/**
 * TikTokPicker — the TikTok video picker for the create flow, free of any
 * route/dialog chrome so it can render inline wherever the host puts it:
 * inside the desktop create dialog, inside the mobile picker page, or on the
 * legacy /upload/tiktok route.
 *
 ...
 *
 * States:
 *  1. Not connected → connect CTA (opens OAuth popup; on success the queries
 *     refetch in place — no full reload, so the host surface keeps its state)
 *  2. Connected, loading → skeleton
 *  3. Connected → single list of the user's TikTok videos (single-select)
 *     → "Use This Video" → createDraftFromTiktokEmbed → store enters the edit
 *     step and `onUsed` fires so the host can continue the flow its own way.
 */
...
export function TikTokPicker({
  ...
  async function handleConnect() {
    ...
    // Refetch (not reload): the picker may be inline inside the create dialog
    // or the mobile picker page, and a reload would throw that state away.
    // Refetching `me` flips isTiktokConnected, which un-skips the videos query.
    const refresh = () => {
      void refetchMe();
    };
